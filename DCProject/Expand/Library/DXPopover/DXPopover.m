...
- (void)showAtPoint:(CGPoint)point
    ...
    NSAssert(CGRectGetWidth(contentView.bounds) > 0 && CGRectGetHeight(contentView.bounds) > 0,
             @"DXPopover contentView bounds.size should not be zero");
    NSAssert(CGRectGetWidth(containerView.bounds) > 0 && CGRectGetHeight(containerView.bounds) > 0,
             @"DXPopover containerView bounds.size should not be zero");
    NSAssert(CGRectGetWidth(containerView.bounds) >= (CGRectGetWidth(contentView.bounds) + self.contentInset.left + self.contentInset.right),
             @"DXPopover containerView width %f should be wider than contentViewWidth %f & "
             @"contentInset %@",
             CGRectGetWidth(containerView.bounds), CGRectGetWidth(contentView.bounds), NSStringFromUIEdgeInsets(self.contentInset));
...
- (void)showAtView:(UIView *)atView withContentView:(UIView *)contentView {
    [self showAtView:atView
        withContentView:contentView
                 inView:[[[UIApplication sharedApplication] windows] objectAtIndex:0].rootViewController.view];
}
...
-(void)presentAtBarButtonItem:(UIBarButtonItem *)barButtonItem withContentView:(UIView *)contentView animated:(BOOL)animated {
    UIView *targetView = (UIView *)[barButtonItem performSelector:@selector(view)];
    UIView *targetSuperview = [targetView superview];
    UIView *containerView = [targetSuperview superview];
    
    NSAssert(containerView, @"Cannot determine container view from UIBarButtonItem: %@",barButtonItem);
    
    [self showAtView:targetView withContentView:contentView inView:containerView];
}
