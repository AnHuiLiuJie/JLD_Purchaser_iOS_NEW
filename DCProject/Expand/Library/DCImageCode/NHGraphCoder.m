...
- (void)setup {
    ...
    self.thumbTintColor = [UIColor dc_colorWithHexString:@"#E1E1E1"];
    self.continuous = YES;
    self.thumbCenterPoint = CGPointZero;
    
    self.backgroundColor = [UIColor whiteColor];
    
     UIImage *thumbImage = [UIImage imageNamed:@"login_anniu"];
    
//    _slider = [[UISlider alloc] initWithFrame:self.bounds]; 修改
    ...
    infoLabel.textColor = [UIColor dc_colorWithHexString:@"#333333"];
    infoLabel.textAlignment = NSTextAlignmentLeft;
    infoLabel.backgroundColor = [UIColor clearColor];
    infoLabel.font = [UIFont fontWithName:PFR size:14];
    infoLabel.text = @"  向右拖动滑块填充拼图";
...
- (void) sliderChanged:(UISlider *)sender {
    ...
    //self.shimmer.alpha = MAX(0.0, 1.0 - (_slider.value * 3.5)); // 修改
    self.shimmer.alpha = MAX(0.0, 1.0 - (_slider.value * 2.5));
    if (_event) {
...
- (void)__initSetup {
    ...
    CGSize size = CGSizeMake(self.bounds.size.width, self.bounds.size.height - 68);
    ...
    bounds = CGRectMake(0, m_icon_size, size.width, 30);
    ...
    bounds = CGRectMake((size.width-m_btn_size)*0.5, m_icon_size*2.5, m_btn_size, 30);
...
- (id)initWithImage:(UIImage *)img {
    ...
    CGSize tmpSize = (CGSize){self.dc_width,self.dc_height - 68};
...
- (void)updateInnerImg:(UIImage *)img {
    ...
    CGSize dstSize = (CGSize){self.dc_width,self.dc_height - 68};
    if (CGSizeEqualToSize(img.size, dstSize)) {
        self.innerImg = img;
        return ;
    }

    CGRect scaledImageRect = CGRectMake(0, 0, self.dc_width, self.dc_height - 68);
...
- (void)__initSetup {
    ...
    CGRect bounds = CGRectMake(self.dc_width-m_btn_size, 0, m_btn_size, m_btn_size);
    ...
    bounds = CGRectMake(0, self.dc_height - 68, self.dc_width, 68);
    UIView * view = [[UIView alloc] initWithFrame:bounds];
    view.backgroundColor = [UIColor whiteColor];
    [self addSubview:view];
    
    bounds = CGRectMake(0, self.dc_height - 36 - 15, self.dc_width , 36);
    UIView * cliView = [[UIView alloc] initWithFrame:bounds];
    cliView.backgroundColor = [UIColor dc_colorWithHexString:@"#E1E1E1"];
    cliView.clipsToBounds = YES;
    cliView.layer.cornerRadius = bounds.size.height/2;
    ...
    bounds = CGRectMake(0, 0, self.dc_width, self.dc_height - 68);
...
- (UIBezierPath *)copyShapePath {
    ...
    tmpBumper->m_center = CGPointMake(PBTile_SIZE/2, PBTile_SIZE/2);
...
- (void)sliderValueChanged:(CGFloat)value {
    ...
    CGRect bounds = CGRectMake((self.dc_width -PBTile_SIZE)*value, self.shapeBumper.m_center.y-PBTile_SIZE*0.5, PBTile_SIZE, PBTile_SIZE);
//    CGRect bounds = CGRectMake(self.dc_width *value, self.shapeBumper.m_center.y-PBTile_SIZE*0.5, PBTile_SIZE, PBTile_SIZE);
    self.mFlag.frame = bounds;
}
//滑动slider结束后调用
- (void)endDetectSliderValue:(CGFloat)value {
    CGFloat tmp_x = (self.dc_width-PBTile_SIZE-PBSLIDER_SIZE)*value;
...
//    int m_ball_radius = PBTile_SIZE/((1+m_ball_cross_scale)+m_radius_scale);
    ...
    int min = PBTile_SIZE;
    int max = self.dc_width - min - 2*min;
...
//    [self.innerImg drawInRect:rect];
    [self.innerImg drawInRect:CGRectMake(0, 0, self.dc_width, self.dc_height - 68)];
...
+ (NHGraphCoder *)codeWithURL:(NSString *)url frame:(CGRect)frame{
    return [[NHGraphCoder alloc] initWithURL:url frame:frame];
}

- (id)initWithURL:(NSString *)url frame:(CGRect)frame{
    NHGraphCoder *coder = [NHGraphCoder new];
    [coder setBounds:frame];
