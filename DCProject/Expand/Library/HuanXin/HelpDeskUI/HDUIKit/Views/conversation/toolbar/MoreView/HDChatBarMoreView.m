...
#define CHAT_BUTTON_SIZE CGSizeMake(50,65)
#define INSETS 10
...
- (void)setupSubviewsForType:(HDChatToolbarType)type
{
    ...
    _photoButton = [self btnWithImage:[UIImage imageNamed:@"hd_chat_image_normal"]
                    highlightedImage:[UIImage imageNamed:@"hd_chat_image_normal"]
                               title:@"相册"];
    ...
    _takePicButton = [self btnWithImage:[UIImage imageNamed:@"hd_chat_takepic_normal"]
                       highlightedImage:[UIImage imageNamed:@"hd_chat_takepic_normal"]
                                  title:@"拍照"];
    ...
    _orderButton = [self btnWithImage:[UIImage imageNamed:@"em_chat_order_normal"]
                         highlightedImage:[UIImage imageNamed:@"em_chat_order_normal"]
                                    title:@"订单"];
    
    [_orderButton setFrame:CGRectMake(insets *3 + CHAT_BUTTON_SIZE.width *2, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];
    [_orderButton addTarget:self action:@selector(takeOrderListAction) forControlEvents:UIControlEventTouchUpInside];
    _orderButton.tag = MOREVIEW_BUTTON_TAG + 2;
    _maxIndex = 2;
    [_scrollview addSubview:_orderButton];


    _evaluationButton = [self btnWithImage:[UIImage imageNamed:@"em_chat_evaluation_normal"]
                          highlightedImage:[UIImage imageNamed:@"em_chat_evaluation_normal"]
                                     title:@"评价"];
    [_evaluationButton setFrame:CGRectMake(insets *4 + CHAT_BUTTON_SIZE.width *3, 10, CHAT_BUTTON_SIZE.width , CHAT_BUTTON_SIZE.height)];

    [_evaluationButton addTarget:self action:@selector(evaluationAction) forControlEvents:UIControlEventTouchUpInside];
    _evaluationButton.tag = MOREVIEW_BUTTON_TAG + 3;
    _maxIndex = 3;
...
- (HDItemButton *)btnWithImage:(UIImage *)aImage highlightedImage:(UIImage *)aHighLightedImage title:(NSString *)aTitle {
    ...
    btn.titleRect = CGRectMake(0, CHAT_BUTTON_SIZE.height - btn.titleLabel.fl_size.height+5, CHAT_BUTTON_SIZE.width, btn.titleLabel.fl_size.height);
    btn.imageRect = CGRectMake(0, 0, CHAT_BUTTON_SIZE.width, CHAT_BUTTON_SIZE.height - btn.titleLabel.fl_size.height);
...
- (void)takeOrderListAction
{
    if (_delegate && [_delegate respondsToSelector:@selector(moreViewOrderListAction:)]) {
        [_delegate moreViewOrderListAction:self];
    }
}
