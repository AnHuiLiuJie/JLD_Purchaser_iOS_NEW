...
NSString *const HDMessageCellIdentifierSendText = @"HDMessageCellSendText";//默认选项，一般为正常的文本消息
NSString *const HDMessageCellIdentifierSendTrack = @"HDMessageCellSendTrack";//// 轨迹消息
NSString *const HDMessageCellIdentifierSendBigExpression = @"HDMessageCellSendBigExpression";//// 大表情消息
NSString *const HDMessageCellIdentifierSendOrder = @"HDMessageCellSendOrder";// // 订单消息
...
+ (void)initialize
{
    ...
    cell.bubbleMaxWidth = (kScreenW-130);
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            case EMMessageBodyTypeText: {
                ...
                switch (extMsgType) {
                    case HDExtTrackMsg:
                        self.contentView.tag = 1993;
                        [_bubbleView setupTrackBubbleView];
                        _bubbleView.delegate = self;
                        break;
                    case HDExtOrderMsg:
                        self.contentView.tag = 1992;
...
//            if (extMsgType == HDExtOrderMsg || extMsgType == HDExtTrackMsg) {
//                return YES; // tag in Cell+Form.h Cell+Order.h   Cell+Track.h
//            }
//        }
        return NO;
    }
    ...
    if (_model.bodyType == EMMessageBodyTypeText) {
        if (touch.view.tag == 1991 || touch.view.tag == 1992 || touch.view.tag == 1993) {
            return YES; // tag in Cell+Form.h Cell+Order.h   Cell+Track.h
        }
        return NO;
    }
...
- (void)setSendBubbleBackgroundImage:(UIImage *)sendBubbleBackgroundImage
{
    ...
    //_sendBubbleBackgroundImage = [UIImage imageNamed:@"sendBubbleBackgroundImage"];
}
    ...
    if (tapRecognizer.state == UIGestureRecognizerStateEnded) {
        ...
        switch (_model.bodyType) {
            case EMMessageBodyTypeText: {
                ...
                }else if([HDMessageHelper getMessageExtType:_model.message] == HDExtOrderMsg){
                    if ([_delegate respondsToSelector:@selector(messageCellSelected:)]) {
                        [_delegate messageCellSelected:_model];
                    }
                }else if([HDMessageHelper getMessageExtType:_model.message] == HDExtTrackMsg){
                    if ([_delegate respondsToSelector:@selector(messageCellSelected:)]) {
                        [_delegate messageCellSelected:_model];
                    }
                }
...
+ (CGFloat)cellHeightWithModel:(id<HDIMessageModel>)model
{
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            CGFloat tableWidth = (kScreenW-130) - cell.bubbleMargin.left - cell.bubbleMargin.right;
            HDExtMsgType extMsgType = [HDMessageHelper getMessageExtType:model.message];
            switch (extMsgType) {
                case HDExtRobotMenuMsg:{
                    HDMenuInfo *info = [model.message menuInfo];
...
//                    return 2 *HDMessageCellPadding + kImageHeight + kTitleHeight + 60;
                    return 2 *HDMessageCellPadding + kImageHeight + kTitleHeight;
                case HDExtOrderMsg:
