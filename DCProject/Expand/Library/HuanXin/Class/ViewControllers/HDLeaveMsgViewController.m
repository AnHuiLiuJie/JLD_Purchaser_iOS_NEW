...
- (void)leaveMessage {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        CGRect frame = self->_backView.frame;
...
//    [parameters setObject:content.length>0 ? content:@"" forKey:@"content"];
    ...
    if (([name isEqualToString:@""] || [tel isEqualToString:@""] || /*[mail isEqualToString:@""] ||*/ [subject isEqualToString:@""] || [content isEqualToString:@""])) {
        UIAlertController *alter = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"prompta", @"Prompt") message:NSLocalizedString(@"Please_fill_out_the_information", @"addinformation") preferredStyle:UIAlertControllerStyleAlert];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (textField.tag == NSTextFieldTagTel && ![string isEqualToString:@""]) {
        if (textField.text.length >=20) {
            UIAlertController *alter = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"prompta", @"Prompt") message:NSLocalizedString(@"message_content_beyond_limit", @"The message content beyond the limit") preferredStyle:UIAlertControllerStyleAlert];
            [alter addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"ok", @"OK") style:UIAlertActionStyleDefault handler:nil]];
            [[UIApplication sharedApplication].keyWindow.rootViewController presentViewController:alter animated:YES completion:nil];
            return NO;
        }
...
- (void)layoutKeyboard {
    ...
    CGFloat offset = self.view.fl_size.height - CGRectGetMaxY(crtRect) - _keyboardHeight;
    
    //将视图上移计算好的偏移
    CGFloat oy = _backView.fl_originY+ offset;
    [UIView animateWithDuration:_duration animations:^{
        self->_backView.frame = CGRectMake(0.0f, oy, self->_backView.frame.size.width, self->_backView.frame.size.height);
    }];
}
