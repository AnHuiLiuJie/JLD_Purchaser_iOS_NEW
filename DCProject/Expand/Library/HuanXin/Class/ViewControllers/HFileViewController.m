...
- (UILabel*)nameLabel
{
    if (_nameLabel == nil) {
        ...
        _nameLabel.fl_centerX = _fileImageView.fl_centerX;
//        _nameLabel.frame.origin.x = _fileImageView.frame.origin.x;
...
- (void)openFileAction
{
    ...
    if (![self.documentInteractionController presentPreviewAnimated:YES]) {
        UIAlertController *alter = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"prompta", @"Prompt") message:NSLocalizedString(@"not_support_preview_files", @"Such systems do not support preview files") preferredStyle:UIAlertControllerStyleAlert];
        [alter addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"know_the", @"Know The") style:UIAlertActionStyleDefault handler:nil]];
    ...
    if (model.bodyType == EMMessageBodyTypeFile) {
        if (body.localPath) {
            ...
            [[HDClient sharedClient].chatManager downloadAttachment:model.message progress:^(int progress) {
                ...
                [self hideHud];
                if (!error) { //成功
                    [weakSelf.downloadButton setTitle:NSLocalizedString(@"have_downloaded", @"Have downloaded") forState:UIControlStateNormal];
                } else { //下载失败
