...
//    self.title = _title;
    ...
    self.chatToolbar = [[HDChatToolbar alloc] initWithFrame:CGRectMake(0, self.view.fl_height - chatbarHeight - LJ_TabbarSafeBottomMargin, self.view.fl_width, chatbarHeight)];
...
- (void)setLeftBarBtnItem {
    ...
    lefbar.action = @selector(backItemClicked);
    lefbar.target = self;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if ([NSStringFromClass([touch.view class]) isEqualToString:@"UITableViewCellContentView"]) {
        if (touch.view.fl_width == 200 || touch.view.tag == 1990) {
            return NO;
        }
    }
    ...
    if(touch.view.tag == 1992 || touch.view.tag == 1993){
        return NO; //tag in Cell+Order.h   Cell+Track.h
    }
    return YES;
}
...
- (void)setChatToolbar:(HDChatToolbar *)chatToolbar
{
    ...
    if (_chatToolbar) {
        ...
        _visitorWaitCountLabel.backgroundColor = [UIColor colorWithWhite:0 alpha:0.3];
    ...
    tableFrame.size.height = self.view.frame.size.height - _chatToolbar.frame.size.height-LJ_TabbarSafeBottomMargin;
...
- (void)_audioMessageCellSelected:(id<HDIMessageModel>)model
{
    ...
    if (model.bodyType == EMMessageBodyTypeVoice) {
        //send the acknowledgement
        __weak HDMessageViewController *weakSelf = self;
        BOOL isPrepare = [[HDMessageReadManager defaultManager] prepareMessageAudioModel:(HDMessageModel*)model updateViewCompletion:^(HDMessageModel *prevAudioModel, HDMessageModel *currentAudioModel) {
...
- (void)messageCellSelected:(id<HDIMessageModel>)model {
    switch (model.bodyType) {
        case EMMessageBodyTypeText: {
            ...
            }else if([HDMessageHelper getMessageExtType:model.message] == HDExtOrderMsg){
                [self _orderMessageCellSelected:model];
            }else if([HDMessageHelper getMessageExtType:model.message] == HDExtTrackMsg){
...
- (void)didHDStartRecordingVoiceAction:(UIView *)micView
{
    ...
    [self _canRecordCompletion:^(HDRecordResponse recordResponse) {
        switch (recordResponse) {
            ...
            case HDCanNotRecord:
            {
                UIAlertController *alter = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:NSLocalizedString(@"message.failToPermission", @"No recording permission") preferredStyle:UIAlertControllerStyleAlert];
...
- (void)moreViewPhotoAction:(HDChatBarMoreView *)moreView//相册
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
...
- (void)moreViewTakePicAction:(HDChatBarMoreView *)moreView//拍照
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
...
//    [self routerEventWithName:HRouterEventTapEvaluate userInfo:@{@"HDMessage":message}];
...
//                NSDictionary *userInfo = @{@"HDMessage":msg};
//                [self routerEventWithName:HRouterEventTapEvaluate userInfo:userInfo];
            }
        });
    }];
...
//    OrderAdvisoryView *view = [[OrderAdvisoryView alloc] initWithFrame:DC_KEYWINDOW.bounds filterModel:nil];
...
- (void)visitorWaitCount:(int)count{
    if (count > 0) {
        if (_visitorWaitCountLabel) {
            ...
            _visitorWaitCountLabel.hidden = YES;//当前排队人数
        }
    }else{
        if (_visitorWaitCountLabel) {
            _visitorWaitCountLabel.hidden = YES;
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo {
    ...
    if ([eventName isEqualToString:HRouterEventTapTransform]) {
        ...
        if ([HDMessageHelper getMessageExtType:message] == HDExtToCustomServiceMsg) {
            ...
            [[HDClient sharedClient].chatManager sendMessage:aHMessage
             ...
             {
                self->_isSendingTransformMessage = NO;
                if (!aError) {
    ...
    if ([eventName isEqualToString:HRouterEventTapEvaluate]) {
        DCSatisfactionView *view = [[DCSatisfactionView alloc] initWithFrame:DC_KEYWINDOW.bounds];
        id <HDIMessageModel> model = nil;
        ...
        [HDClient.sharedClient.chatManager asyncPostRobotQuality:msg
                                                      ...
                                                      completion:^(NSDictionary *info, HDError *error)
         {
            ...
            hd_dispatch_main_async_safe(^(){
                [self hideHud];
                if (!error) {
                    HDMessage *needUpdateMsg = [self.conversation loadMessageWithId:msg.messageId error:nil];
    ...
    [HDClient.sharedClient.chatManager asyncPostRobotQuality:aMsg
                                                  ...
                                                  completion:^(NSDictionary *info, HDError *error)
     {
        ...
        hd_dispatch_main_async_safe(^(){
            [self hideHud];
            if (!error) {
                HDMessage *needUpdateMessage = [self->_conversation loadMessageWithId:msg.messageId error:nil];
...
- (void)commitSatisfactionWithControlArguments:(ControlArguments *)arguments
                                   ...
                                   reasonArray:(NSMutableArray *)reasonArray
    ...
    NSString *detail = @"";
    NSInteger reason = 0;
    for (NSDictionary *dic in reasonArray) {
        if (detail.length > 0 ) {
            detail = [NSString stringWithFormat:@"%@,%@",detail,[dic objectForKey:@"name"]];
        }else
            detail = [NSString stringWithFormat:@"%@%@",detail,[dic objectForKey:@"name"]];
        
        NSString *appraiseTagsId = [dic objectForKey:@"id"];
        NSInteger indexTag = [appraiseTagsId integerValue];
        reason = reason + indexTag;
    }
    if (detail.length > 0) {
        [ctrlArgs setValue:detail forKey:@"detail"];
    }
    ...
    [[HDClient sharedClient].chatManager sendMessage:message
                                          ...
                                          completion:^(HDMessage *aMessage, HDError *aError)
     {
        ...
        if (!aError) {
            NSString *isSolve = @"1";
            if (tags.count != 0) {
                NSDictionary *dic = tags[0];
                if ([[dic objectForKey:@"name"] isEqualToString:@"未解决"]) {
                    isSolve = @"2";
                }
            }
            NSString *star = [ctrlArgs objectForKey:@"summary"];
            [[DCAPIManager shareManager] pioneerRequest_b2c_common_customer_evalAddWithCustomerId:message.conversationId dissatisfiedReason:[self getDissatisfiedReason:reason] isSolve:isSolve star:star success:^(id response) {
                [weakself showHint:@"感谢您的评价！您的支持是我们的动力！"];
...
- (NSString *)getDissatisfiedReason:(NSInteger)reason
{
    //不满意原因：100-回复不及时，010-服务态度差，001-客服对业务不熟悉（111-表示三个原因都选中）
    NSString *dissatisfiedReason = @"000";
    if (reason == 100) {
        dissatisfiedReason = @"100";
    }else if(reason == 10){
        dissatisfiedReason = @"010";
    }else if(reason == 1){
        dissatisfiedReason = @"001";
    }else if(reason == 110){
        dissatisfiedReason = @"110";
    }else if(reason == 101){
        dissatisfiedReason = @"101";
    }else if(reason == 111){
        dissatisfiedReason = @"111";
    }else if(reason == 11){
        dissatisfiedReason = @"011";
    }
