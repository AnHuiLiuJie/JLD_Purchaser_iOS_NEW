...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.addImage = [UIImage imageNamed:@"human_my_photo_add"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.deleteBtnTapBlock = ^(YNImageUploadViewCollCell * _Nonnull deleCell) {
        ...
        //weakSelf.YNImageUploadViewl_delBlock ? : weakSelf.YNImageUploadViewl_delBlock();
        !weakSelf.YNImageUploadViewl_FinishBlock ? : weakSelf.YNImageUploadViewl_FinishBlock(@[]);
    };
    cell.YNImageUploadViewCollCell_FinishBlock = ^(NSArray * _Nonnull models) {
        !weakSelf.YNImageUploadViewl_FinishBlock ? : weakSelf.YNImageUploadViewl_FinishBlock(models);
    };
    return cell;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.isLast) {
        ...
        imagePicker.maxImagesCount = 18;/*lj change*/
        imagePicker.selectedAssets = marray;
        [imagePicker setDidFinishPickingPhotosHandle:^(NSArray<UIImage *> *photos, NSArray *assets, BOOL isSelectOriginalPhoto) {
            //NSMutableArray *marray = @[].mutableCopy;
            /*lj change*/
            NSMutableArray *newArr = [self.dataList mutableCopy];
            YNImageModel *lastModel = [newArr lastObject];
            if (lastModel.imageUrl.length == 0) {
                [newArr removeLastObject];
            }
            NSMutableArray *marray = newArr.mutableCopy;
...
- (void)layoutSubviews {
    ...
    if (self.config.isAutoHeight) {
        ...
        !_addPicturesViewClickBlock ?  : _addPicturesViewClickBlock(self.viewHeight);
    }
    ...
    if (!_dataList) {
        ...
        if (self.config.type == YNImageUploadImageTypeUpload) {
            ...
            for (int i = 0; i < self.config.contents.count; i ++) {
                ...
                if (i == self.config.contents.count-1 && self.config.type == YNImageUploadImageTypeImageUrl) {
                    [self.dataList addObject:[self lastImageModel]];
                }/*lj change*/
            }
        }
