...
- (void)setUpRollingLeft
{
    ...
    self.leftImageView.frame = CGRectMake(0, 0, 60, RollingViewHeight - 0);
...
- (void)setUpRollingCenterRight
{
    ...
    if (_rolTitles.count > 1) {//@[@[],@[]]
        ...
        for (NSInteger i = 0; i < firstTitleArrray.count; i++) {
            ...
            UIButton *middleView = [self getBackMiddleViewWithFrame:CGRectMake(CGRectGetMaxX(self.leftImageView.frame) + 5, 0, RollingViewWidth - CGRectGetMaxX(self.leftImageView.frame), RollingViewHeight) WithIndex:i]; //中间View
            ...
            if (_rolTags.count > 0 && firstTitleArrray.count == firstTagArrray.count) {
                ...
                tagTopLabel.textColor = tagBottomLabel.textColor = [UIColor dc_colorWithHexString:@"#FF5B11"];
                ...
                if (self.isShowTagBorder) { //是否tag显示边框
                    [self dc_changeControlCircularWith:tagTopLabel AndSetCornerRadius:3 SetBorderWidth:1 SetBorderColor:tagTopLabel.textColor canMasksToBounds:YES];
                    [self dc_changeControlCircularWith:tagBottomLabel AndSetCornerRadius:3 SetBorderWidth:1 SetBorderColor:tagBottomLabel.textColor canMasksToBounds:YES];
                }
    ...
    if (_rolTitles.count > 0) {
        for (NSInteger i = 0; i < _rolTitles.count; i++) {
            ...
            if (self.rolTags.count > 0 && self.rolTitles.count == self.rolTags.count) {
                ...
                if (self.isShowTagBorder) {//是否tag显示边框
                    [self dc_changeControlCircularWith:tagLabel AndSetCornerRadius:3 SetBorderWidth:1 SetBorderColor:tagLabel.textColor canMasksToBounds:YES];
                }
...
#pragma mark - 点击更多
- (void)rightMoreButtonClick:(UIButton *)button
{
