...
- (void)setupDefault
{
    // 1.设置数据的默认值
    _title            = @"                       ";
    _STPFont             = [UIFont systemFontOfSize:17];
...
- (void)setSTPFont:(UIFont *)STPFont
{
    _STPFont = STPFont;
    [self.buttonLeft.titleLabel setFont:STPFont];
    [self.buttonRight.titleLabel setFont:STPFont];
    [self.labelTitle setFont:STPFont];
}
...
- (UIView *)contentView
{
    if (!_contentView) {
        ...
        [_contentView setBackgroundColor:[UIColor dc_colorWithHexString:@"#FAFAF8"]];
        _contentView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
        [_contentView dc_cornerRadius:10 rectCorner:UIRectCornerTopLeft | UIRectCornerTopRight ];
    }
    return _contentView;
}
...
- (UIPickerView *)pickerView
{
    if (!_pickerView) {
        ...
        _pickerView.tintColor = [UIColor dc_colorWithHexString:@"#292929"];
...
//        [_buttonLeft addBorderColor:self.borderButtonColor];
        [_buttonLeft.titleLabel setFont:self.STPFont];
...
//        [_buttonRight addBorderColor:self.borderButtonColor];
        [_buttonRight.titleLabel setFont:self.STPFont];
...
- (UILabel *)labelTitle
{
    if (!_labelTitle) {
        ...
        [_labelTitle setFont:self.STPFont];
