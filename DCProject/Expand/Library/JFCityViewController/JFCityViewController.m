...
- (void)viewDidLoad {
    ...
    if ([kCurrentCityInfoDefaults objectForKey:@"allCityData"]) {
        self.characterMutableArray = [NSKeyedUnarchiver unarchiveObjectWithData:[kCurrentCityInfoDefaults objectForKey:@"allCityData"]];
        _sectionMutableArray = [NSKeyedUnarchiver unarchiveObjectWithData:[kCurrentCityInfoDefaults objectForKey:@"allSectionData"]];
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        _searchView = [[JFSearchView alloc] initWithFrame:CGRectMake(0, 80, frame.size.width, frame.size.height  - 80 - kNavBarHeight)];
...
- (NSArray *)hotCityArray {
    if (!_hotCityArray) {
        _hotCityArray = @[@"合肥", @"芜湖", @"郑州", @"武汉", @"西安", @"南京", @"成都", @"重庆" , @"北京"];
    }
...
- (void)processData:(void (^) (id))success {
    ...
    [kCurrentCityInfoDefaults setValue:cityData forKey:@"allCityData"];
    [kCurrentCityInfoDefaults setObject:sectionData forKey:@"allSectionData"];
    success(@"成功");
}
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        _rootTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreenW, kScreenH - kNavBarHeight) style:UITableViewStylePlain];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger line = 0;//热门城市行数
    if (self.hotCityArray && self.hotCityArray.count > 0) {
        line = ((_hotCityArray.count - 1) / 3) + 1;
        
    }
    
    return indexPath.section == (_HeaderSectionTotal - 1) ? (40+10)*line : 44;
}
