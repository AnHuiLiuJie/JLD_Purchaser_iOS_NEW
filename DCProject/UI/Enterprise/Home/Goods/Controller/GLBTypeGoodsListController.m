...
@interface GLBTypeGoodsListController ()
...
@implementation GLBTypeGoodsListController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    
    if (self.goodsType == GLBGoodsTypeRmsx) {
        self.navigationItem.title = @"热卖上新";
    } else if (self.goodsType == GLBGoodsTypeYhcx) {
        self.navigationItem.title = @"优惠促销";
    } else if (self.goodsType == GLBGoodsTypeYkx) {
        self.navigationItem.title = @"药控销";
    } else if (self.goodsType == GLBGoodsTypeYzb) {
        self.navigationItem.title = @"药招标";
    } else if (self.goodsType == GLBGoodsTypeYjk) {
        self.navigationItem.title = @"药健康";
    }
    ...
    if (self.goodsName) {
        self.searchBar.searchTF.text = self.goodsName;
    }
...
- (void)dc_filtrateViewBtnClick:(NSInteger)index
{
    ...
    if (index == 1) { // 分类
        ...
        vc.frameType = [NSString stringWithFormat:@"%d",kNavBarHeight + 40 + 36];
        vc.cancelBlock = ^{
...
//        [self presentViewController:vc animated:NO completion:nil];
    ...
    if (index == 3) { // 商家
        ...
        vc.searchName = self.searchBar.searchTF.text;
        vc.catIds = catIds;
        vc.frameType = [NSString stringWithFormat:@"%d",kNavBarHeight + 40 + 36];
        vc.cancelBlock = ^{
            [weakSelf.filtrateView dc_recoverInit];
...
- (void)setUpTableView
{
    ...
    self.tableView.frame = CGRectMake(0, kNavBarHeight + 40 + 36, kScreenW, kScreenH - kNavBarHeight - 40 - 36);
...
- (GLBGoodsFiltrateView *)filtrateView{
    if (!_filtrateView) {
        _filtrateView = [[GLBGoodsFiltrateView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.searchBar.frame), kScreenW, 40)];
        WEAKSELF;
