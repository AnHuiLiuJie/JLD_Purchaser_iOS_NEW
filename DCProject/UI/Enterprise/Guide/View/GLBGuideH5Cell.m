...
//    NSString *ipStr = [DCObjectManager dc_readUserDataForKey:DC_H5_Key];
    
    NSString *ipStr = @"该方法已经被弃用";
    
    _ipLabel = [[UILabel alloc] init];
    _ipLabel.textColor = [UIColor dc_colorWithHexString:@"#333333"];
    _ipLabel.font = PFRFont(14);
    _ipLabel.text = [NSString stringWithFormat:@"当前h5地址：%@",ipStr];
    [self.contentView addSubview:_ipLabel];
    
    _textField = [[DCTextField alloc] init];
    _textField.placeholder = [NSString stringWithFormat:@"请输入h5地址，如：%@",DC_H5BaseUrl];
...
//    [DCObjectManager dc_saveUserData:self.textField.text forKey:DC_H5_Key];
    
    _ipLabel.text = [NSString stringWithFormat:@"当前h5地址：%@",self.textField.text];
