...
@interface GLBGuidePersonIPCell ()
...
@implementation GLBGuidePersonIPCell
...
//    NSString *ipStr = [DCObjectManager dc_readUserDataForKey:DC_PersonIP_Key];
    
    NSString *ipStr = @"该方法已经废弃";
    
    _ipLabel = [[UILabel alloc] init];
    _ipLabel.textColor = [UIColor dc_colorWithHexString:@"#333333"];
    _ipLabel.font = PFRFont(14);
    _ipLabel.text = [NSString stringWithFormat:@"个人地址：%@",ipStr];
    [self.contentView addSubview:_ipLabel];
    
    _textField = [[DCTextField alloc] init];
    _textField.placeholder = @"请输入请求地址，如：http://192.168.0.106:8086/b2c";
...
//    [DCObjectManager dc_saveUserData:self.textField.text forKey:DC_PersonIP_Key];
    
    _ipLabel.text = [NSString stringWithFormat:@"个人地址：%@",self.textField.text];
