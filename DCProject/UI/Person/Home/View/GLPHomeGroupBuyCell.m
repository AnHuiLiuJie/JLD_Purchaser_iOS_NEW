...
#pragma mark - Intial
@implementation GLPHomeGroupBuyCell
...
- (void)setUpUI
{
    ...
    _bgView.backgroundColor = [UIColor dc_colorWithHexString:@"#F9F4EB"];
    ...
    _iconImg.image = [UIImage imageNamed:@"dc_home_rexiao"];
...
-(void)setCollageList:(NSArray<DCCollageListModel *> *)collageList{
    if ([collageList isEqualToArray:_collageList]) {
        _isFirst = YES;
    }else
        _isFirst = NO;
    
    _collageList = collageList;
    
    _topBgImg.image = [UIImage imageNamed:@"dc_home_GroupBuy"];

    //_subtitleLab.text = @"";
    
    if (_collageList.count > 0 && !_isFirst) {
        for(UIView *view in [self.scrollBgView subviews]){
            if ([view isKindOfClass:[GLPHomeGroupBuyGoodsView class]]) {
                [view removeFromSuperview];
            }
        }
        _isFirst = YES;
        for (int i=0; i<_collageList.count; i++) {
            DCCollageListModel *model = _collageList[i];
            GLPHomeGroupBuyGoodsView *view = [[GLPHomeGroupBuyGoodsView alloc] init];
        ...
        self.scrollBgView.contentSize = CGSizeMake((_itemW+_spacing)*_collageList.count, _scrollBgViewH);
    }
...
- (void)seeBtnClick:(UIButton *)button{
    !_GLPHomeGroupBuyCell_moreBlock ? : _GLPHomeGroupBuyCell_moreBlock();
}

- (void)tapAction:(UITapGestureRecognizer *)recognizer{
    UIView *view = recognizer.view;
    DCCollageListModel *model = _collageList[view.tag];
    !_GLPHomeGroupBuyCell_clickGoodsBlock ? : _GLPHomeGroupBuyCell_clickGoodsBlock(model);
}
...
@interface GLPHomeGroupBuyGoodsView ()
...
@implementation GLPHomeGroupBuyGoodsView
...
#pragma mark - setter
- (void)setListModel:(DCCollageListModel *)listModel
{
...
//        }
    ...
    _priceLabel.text = [NSString stringWithFormat:@"拼团价¥%@",_listModel.price];
    _priceLabel = [UILabel setupAttributeLabel:_priceLabel textColor:_priceLabel.textColor minFont:[UIFont fontWithName:PFR size:10] maxFont:[UIFont fontWithName:PFRMedium size:15] forReplace:@"拼团价¥"];
}
