...
@interface TRClassGoodsVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITextFieldDelegate,YBPopupMenuDelegate,UITextFieldDelegate>
...
@property (nonatomic,strong) UIButton *good_Bt;
/**
 0：列表视图，1：格子视图
 */
@property (nonatomic, assign) BOOL isSwitchGrid;
@property(nonatomic,strong) UICollectionView *collectionView;
@property(nonatomic,strong) NoGoodsHeaderView *headerView;
@property (nonatomic, assign) BOOL ishaveGoodsList;//goodsList为空时显示hotGoodsList内容
@end

static NSString *const NoGoodsHeaderViewID = @"NoGoodsHeaderView";
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _collectionView.frame = CGRectMake(0, kStatusBarHeight+111,kScreenW, kScreenH-kStatusBarHeight-LJ_TabbarSafeBottomMargin-111-LJ_TabbarSafeBottomMargin);
        _collectionView.showsVerticalScrollIndicator = NO;
        _collectionView.delegate = self;
        _collectionView.dataSource = self;
        [_collectionView registerClass:[NoGoodsHeaderView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NoGoodsHeaderViewID]; //头部View
...
- (void)viewDidLoad {
    ...
    self.searTF.attributedPlaceholder = [NSString dc_placeholderWithString:@"搜索你需要的商品"];
    self.searTF.textColor = [UIColor blackColor];
    _totalinter = 0;
    self.topHeight.constant = kStatusBarHeight;
    self.searView.layer.masksToBounds = YES;
    self.searView.layer.cornerRadius = 15;
    ...
    self.page = 1;
    self.isFirst = @"0";
    self.searTF.text=self.searchStr;
    [self getGoodsList];
    ...
    _good_Bt = [UIButton buttonWithType:(UIButtonTypeCustom)];
    ...
    [_good_Bt addTarget:self action:@selector(addMoreGoodsAction) forControlEvents:(UIControlEventTouchUpInside)];
...
- (void)getGoodsList
{
    ...
    if ([self.firmId dc_isNull]||self.firmId.length==0)
    {//整个app搜索或者分类商品接口
        WEAKSELF;
        [[DCAPIManager shareManager]person_getClassGoodsListywithcatId:[NSString stringWithFormat:@"%@",self.classId] brandId:@"" goodsTagNameList:self.goodsTagNameList brandName:brandStr currentPage:[NSString stringWithFormat:@"%d",self.page] descFlag:self.descFlag dosageForm:dosageFormStr goodsIds:@"" goodsTitle:@"" manufactory:manufactoryStr orderFlag:self.orderFlag searchName:self.searTF.text symptom:symptomStr useMethod:wayStr usePerson:peopleStr success:^(id response) {
            ...
            weakSelf.ishaveGoodsList = YES;
            if (arr.count == 0) {
                arr = response[@"data"][@"hotGoodsList"];
                weakSelf.ishaveGoodsList = NO;
            }
            ...
            if ([weakSelf.isFirst isEqualToString:@"0"])
            {
                NSDictionary *typeDic = response[@"data"][@"facetList"];
                NSArray *brandarr = typeDic[@"brandName"];
                [weakSelf.brandArray addObjectsFromArray:brandarr];
                NSArray *venderarr = typeDic[@"manufactoryStr"];
                [weakSelf.venderArray addObjectsFromArray:venderarr];
                NSArray *dosagarr = typeDic[@"dosageForm"];
                [weakSelf.dosagformArray addObjectsFromArray:dosagarr];
                NSArray *symptonarr = typeDic[@"symptom"];
                [weakSelf.symptomsArray addObjectsFromArray:symptonarr];
                NSArray *peoplearr = typeDic[@"usePerson"];
                [weakSelf.userpeopleArray addObjectsFromArray:peoplearr];
                NSArray *wayarr = typeDic[@"useMethod"];
                [weakSelf.userwayArray addObjectsFromArray:wayarr];
                weakSelf.isFirst = @"1";
            }
        ...
        //店铺搜索商品接口
        WEAKSELF;
        [[DCAPIManager shareManager]person_getStoreGoodsListywithfirmId:[NSString stringWithFormat:@"%@",self.firmId] brandId:@"" brandName:brandStr currentPage:[NSString stringWithFormat:@"%d",self.page] descFlag:self.descFlag dosageForm:dosageFormStr goodsIds:@"" goodsTitle:@"" manufactory:manufactoryStr orderFlag:self.orderFlag searchName:self.searTF.text symptom:symptomStr useMethod:wayStr usePerson:peopleStr success:^(id response) {
            if (weakSelf.page == 1) {
                [weakSelf.dataArray removeAllObjects];
            }
            NSString *allpagestr = [NSString stringWithFormat:@"%@",response[@"data"][@"totalPage"]];
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if (kind == UICollectionElementKindSectionHeader){
        _headerView = [collectionView dequeueReusableSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:NoGoodsHeaderViewID forIndexPath:indexPath];
        WEAKSELF;
        _headerView.NoGoodsHeaderView_block = ^{
            [weakSelf addMoreGoodsAction];
        };
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section {
    ...
    if (_ishaveGoodsList) {
        self.headerView.hidden = YES;
        _good_Bt.hidden = NO;
        _collectionView.frame = CGRectMake(0, kStatusBarHeight+111,kScreenW, kScreenH-kStatusBarHeight-LJ_TabbarSafeBottomMargin-111);
        return CGSizeMake(kScreenW, 4); //头部
    }else{
        self.headerView.hidden = NO;
        _good_Bt.hidden = YES;
        _collectionView.frame = CGRectMake(0, kNavBarHeight,kScreenW, kScreenH-kNavBarHeight-LJ_TabbarSafeBottomMargin);
        return CGSizeMake(kScreenW, 160); //头部
    }
}
