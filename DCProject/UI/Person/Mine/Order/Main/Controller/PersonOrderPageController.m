...
@interface PersonOrderPageController ()<UITextFieldDelegate>
...
@implementation PersonOrderPageController
...
- (void)backClick1
{
    if(self.orderNo_str || self.isPopView) {
...
- (UIViewController *)pageController:(WMPageController *)pageController viewControllerAtIndex:(NSInteger)index {
    PersonOrderListController *vc = [PersonOrderListController new];
...
- (NSArray *)types{
    if (!_types) {
        if (self.isRefund) {
            ...
            _types = @[@(GLPOrderTypeAll),
                       @(GLPOrderTypePay),
                       @(GLPOrderTypeSend),
                       @(GLPOrderTypeAccept),
                       @(GLPOrderTypeEvaluate)];
