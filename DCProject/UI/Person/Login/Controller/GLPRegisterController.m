...
@interface GLPRegisterController ()<DCCodeButtonDelegate>
...
@implementation GLPRegisterController
...
- (void)registerBtnClick:(UIButton *)button
{
    ...
    MJWeakSelf
    [[DCAPIManager shareManager]person_requestResginWithcellphone:self.phoneTF.text loginName:self.phoneTF.text loginPwd:self.aginTF.text tempUserId:userId validInfo:self.smsCodeTF.text success:^(id response) {
        NSDictionary *dict = @{@"registerNumber":@"注册次数"};//UM统计 自定义搜索关键词事件
        [MobClick event:UMEventCollection_4 attributes:dict];
    ...
    [CaptchaView showWithType:puzzle CompleteBlock:^(NSString *  result) {
        ...
        [[DCAPIManager shareManager] dc_request_b2c_common_captcha_sendMessageWithDic:patameters Success:^(id  _Nullable response) {
            if ([response[DC_ResultCode_Key] integerValue] == DC_Result_Success) {
                ...
                //已注册
                [[DCAlterTool shareTool] showCustomWithTitle:@"" message:@"该手机号已注册,是否前往登录" customTitle1:@"登录" handler1:^(UIAlertAction * _Nonnull action) {
                    !weakSelf.GLPRegisterController_block ? : weakSelf.GLPRegisterController_block(weakSelf.phoneTF.text);
...
- (void)setUpUI
{
    ...
    _aginTF.keyboardType=UIKeyboardTypeNumbersAndPunctuation;
