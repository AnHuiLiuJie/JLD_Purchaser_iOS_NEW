...
@interface GLPTabBarController () <UITabBarControllerDelegate,HDChatManagerDelegate>
...
- (void)viewDidLoad {
    ...
    NSArray *array = @[@{
                            ControllerClassKey:NSStringFromClass([GLPHomeViewController class]),
                            TitleNameKey:@"首页",
                            ImgNormalKey:@"shouye",
                            ImgSelectKey:@"shiyey2"},
                        
                        @{
                            ControllerClassKey:NSStringFromClass([GLPClassController class]),
                            TitleNameKey:@"分类",
                            ImgNormalKey:@"fenlei",
                            ImgSelectKey:@"fenlie2"},
                        
                        @{
                            ControllerClassKey:NSStringFromClass([GLPShoppingCarController class]),
                            TitleNameKey:@"购物车",
                            ImgNormalKey:@"gouwu",
                            ImgSelectKey:@"gouwuc"},
                        
                        @{
                            ControllerClassKey:NSStringFromClass([GLPMineController class]),
                            TitleNameKey:@"个人中心",
                            ImgNormalKey:@"wode",
                            ImgSelectKey:@"wode2"}
    ...
    if (goods.length >1) {
        ...
        dispatch_after(timer, dispatch_get_main_queue(), ^(void){
            [self dc_pushPersonStoreController:goods];
        });
    }
...
- (void)_showNotificationWithMessage:(NSArray *)messages
{
    ...
    if (NSClassFromString(@"UNUserNotificationCenter")) {
        ...
        //[SVProgressHUD showInfoWithStatus:[NSString stringWithFormat:@"收到推送%@",messageModel.messageId]];
        notification.fireDate = [NSDate date]; //触发通知的时间
...
//            {
//                //lj_will_change
//                if (![obj isKindOfClass:[HDChatViewController class]])
//                {
//                    [self.navigationController popViewControllerAnimated:NO];
//                }
//                else
//                {
//                    NSString *conversationChatter = userInfo[DC_kConversationChatter];
//                    HDChatViewController *chat = (HDChatViewController *)obj;
//                    if (![chat.conversation.conversationId isEqualToString:conversationChatter])
//                    {
//                        [self.navigationController popViewControllerAnimated:NO];
//                        //EMMessageBodyType messageType = [userInfo[DC_kMessageType] intValue];
//                        chat = [[HDChatViewController alloc] initWithConversationChatter:@""];
//
////                        chatViewController = [[HDChatViewController alloc] showMenuViewController:conversationChatter andIndexPath:nil messageType:messageType];
////                        chatViewController = [[HDChatViewController alloc] initWithConversationChatter:conversationChatter conversationType:[self conversationTypeFromMessageType:messageType]];
