...
- (void)awakeFromNib {
    ...
    //圆角
    [self dc_changeControlCircularWith:_bgView AndSetCornerRadius:3 SetBorderWidth:0 SetBorderColor:[UIColor whiteColor] canMasksToBounds:YES];
...
- (void)setModel:(TRStoreGoodsModel *)model
{
    ...
    _store_H_LayoutConstraint.constant = 1;
...
//    _storeLab.hidden = NO;
    ...
    if (_model.frontClassName.length > 0) {
        ...
        if (title_w>40) {
            _calssView_W_LayoutConstraint.constant = 48;
            _nameLab.text = [NSString stringWithFormat:@"            %@",_model.goodsTitle];
        }else if(title_w>25 && title_w<40){
...
- (void)create_typeLabelOnView:(NSArray *)array
{
    ...
    for (NSString *str in array) {
        ...
        if (index_w > self.typeView.dc_width) {
            break;
        }
